      ...
      try {
        refinedResults = await callGeminiWithModelFallback(apiKey, async (ai, modelName) => {
          const refinePrompt = `You are a native Kurdish Sorani subtitle localization and refinement specialist.
            ...
            4. RTL & PUNCTUATION INTEGRITY: This is a strict RTL language. FOR VIDEO PLAYER COMPATIBILITY: If a sentence or line ends with any punctuation mark (such as ',', '،', '.', '...', '?', '؟', '!', '؛'), move that punctuation mark to the ABSOLUTE START of the Kurdish line (e.g. '.سڵاو' instead of 'سڵاو.'). This ensures that on RTL video players, the punctuation renders on the correct side visually.
